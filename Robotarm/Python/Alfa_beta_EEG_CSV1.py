...
class Graph:
    def __init__(self, board_shim):
        ...
        filename = time.strftime("eeg_log_%Y%m%d-%H%M%S.csv")
        self.csv_path = os.path.join(log_dir, filename)

        with open(self.csv_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            header = ['absolute_time', 'session_time (s)'] + channel_names
    ...
    def update(self):
        ...
        # === CSV Logging: gem nyeste måling fra alle EEG-kanaler
        timestamp = time.time()
        session_time = timestamp - self.session_start
        scaling = 0.02235  # for Cyton default settings
        latest_values = [data[ch][-1] * scaling for ch in self.exg_channels]


        with open(self.csv_path, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([timestamp, round(session_time, 3)] + latest_values)

        # === Signalbehandling og band power
        ...
        self.win.setWindowTitle(f"Alpha Power ({alpha_channel_name}): {alpha_value:.2f} µV²/Hz")
        ...
        # === Opdater band-graf med rigtig tid
        if 'Alpha' in band_powers:
            self.band_timestamps.append(time.time())
            ...
            t0 = self.band_timestamps[0] if self.band_timestamps else time.time()
