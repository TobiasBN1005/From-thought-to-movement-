...
def train_and_evaluate_kfold(eeg_list, emg_list, subject_ids, create_model_fn,
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X, y, groups=subject_ids_arr)):
        ...
        model_dir = f"saved_models_LSTM_RNN"
        os.makedirs(model_dir, exist_ok=True)
        model.save(os.path.join(model_dir, f"LSTM_RNN_fold{fold+1}.keras"))
    ...
    csv_path = os.path.join(model_dir, "LSTM_RNN_kfold_results.csv")
...
if __name__ == "__main__":
    try:
        results = train_and_evaluate_kfold(
            EEG_DATA, EMG_DATA, SUBJECT_IDS, create_lstm_rnn_model,
