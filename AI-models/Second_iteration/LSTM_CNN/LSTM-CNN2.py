...
def plot_training_curves(history, fold_num, save_dir="training_curves_LSTM"):
...
def create_lstm_cnn_model(input_shape, num_classes=2):
    ...
    conv1 = tf.keras.layers.Conv1D(filters=32, kernel_size=2, padding='same', activation='relu')(x)
    ...
    conv3 = tf.keras.layers.Conv1D(filters=128, kernel_size=5, padding='same', activation='relu')(conv2)
...
def train_and_evaluate_kfold(eeg_list, emg_list, subject_ids, create_model_fn,
                             n_splits=5, window_size=500, epochs=50, batch_size=32):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X, y, groups=subject_ids_arr)):
        ...
        model_dir = f"saved_models_LSTM_CNN"
        os.makedirs(model_dir, exist_ok=True)
        model.save(os.path.join(model_dir, f"LSTM_CNN_fold{fold+1}.keras"))
    ...
    csv_path = os.path.join(model_dir, "LSTM_CNN_kfold_results.csv")
...
if __name__ == "__main__":
    try:
        results = train_and_evaluate_kfold(
            EEG_DATA, EMG_DATA, SUBJECT_IDS, create_lstm_cnn_model,
